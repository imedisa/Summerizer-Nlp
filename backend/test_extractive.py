from extractive import textrank_summarize, summarize_from_file
import os


def test_with_sample_text():
    """تست با یک متن نمونه"""
    print("=" * 80)
    print("تست ۱: خلاصه‌سازی متن نمونه")
    print("=" * 80)
    
    text = """
    هوش مصنوعی یکی از مهم‌ترین فناوری‌های قرن بیست‌ویکم است. 
    این فناوری توانسته است در حوزه‌های مختلفی مانند پزشکی، صنعت و آموزش تحول ایجاد کند.
    گوگل اخیراً مدل زبانی جدید خود به نام جمینای را معرفی کرد که قابلیت پردازش همزمان متن، تصویر و صدا را دارد.
    در ایران نیز استارتاپ‌های مختلفی در حوزه پردازش زبان طبیعی فارسی فعالیت می‌کنند و محصولاتی مانند چت‌بات‌های هوشمند ارائه می‌دهند.
    کارشناسان معتقدند که هوش مصنوعی تا سال ۲۰۳۰ بیش از ۱۵ درصد از تولید ناخالص داخلی جهان را تحت تأثیر قرار خواهد داد.
    با این حال، نگرانی‌هایی درباره اخلاق و امنیت داده‌ها در استفاده از این فناوری‌ها وجود دارد.
    دولت‌های مختلف در حال تدوین قوانین و مقررات برای نظارت بر توسعه هوش مصنوعی هستند.
    محققان همچنین بر نیاز به شفافیت در الگوریتم‌های یادگیری ماشین تأکید دارند.
    """
    
    result = textrank_summarize(text, summary_ratio=0.3)
    print_result(result)
    
    print("\n" + "-" * 80)
    print("تست با تعداد دقیق 3 جمله:")
    print("-" * 80)
    result2 = textrank_summarize(text, num_sentences=3)
    print_result(result2)


def test_with_file():
    """تست با فایل sample_texts.txt"""
    print("\n" + "=" * 80)
    print("تست ۲: خلاصه‌سازی از فایل")
    print("=" * 80)
    
    file_path = "sample_texts.txt"
    
    if not os.path.exists(file_path):
        print(f"❌ فایل {file_path} یافت نشد!")
        return
    
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()
    
    news_items = content.split('===')
    
    news_items = [item.strip() for item in news_items if item.strip() and 'خبر' in item]
    
    for i, news in enumerate(news_items[:3], 1):  
        lines = news.split('\n')
        title = lines[0].strip() if lines else f"خبر {i}"
        text = '\n'.join(lines[1:]).strip()
        
        if len(text) > 50: 
            print(f"\n{'─' * 80}")
            print(f"📰 {title}")
            print('─' * 80)
            
            result = textrank_summarize(text, summary_ratio=0.3)
            print_result(result, show_original=False)


def test_different_ratios():
    """تست با نسبت‌های مختلف"""
    print("\n" + "=" * 80)
    print("تست ۳: مقایسه نسبت‌های مختلف")
    print("=" * 80)
    
    text = """
    پردازش زبان طبیعی شاخه‌ای از هوش مصنوعی است که به کامپیوترها کمک می‌کند زبان انسانی را درک کنند.
    این فناوری کاربردهای زیادی دارد از جمله ترجمه ماشینی، تشخیص گفتار، و پاسخ به سوالات.
    خلاصه‌سازی متن یکی از چالش‌های مهم در این حوزه است.
    دو روش اصلی برای خلاصه‌سازی وجود دارد: استخراجی و مولد.
    روش استخراجی جملات مهم را از متن اصلی انتخاب می‌کند.
    روش مولد جملات جدیدی تولید می‌کند که محتوای اصلی را بیان می‌کنند.
    الگوریتم TextRank یکی از روش‌های محبوب برای خلاصه‌سازی استخراجی است.
    این الگوریتم بر پایه PageRank گوگل طراحی شده است.
    """
    
    ratios = [0.2, 0.3, 0.5]
    
    for ratio in ratios:
        print(f"\n{'─' * 80}")
        print(f"نسبت خلاصه: {ratio:.0%}")
        print('─' * 80)
        
        result = textrank_summarize(text, summary_ratio=ratio)
        print(f"تعداد جملات: {result['num_summary_sentences']} از {result['num_original_sentences']}")
        print(f"خلاصه: {result['summary']}")


def print_result(result, show_original=True):
    """چاپ نتایج به صورت زیبا"""
    if 'error' in result:
        print(f"❌ خطا: {result['error']}")
        return
    
    if show_original:
        print(f"\n📄 متن اصلی:")
        print(result['original_text'].strip()[:200] + "...")
    
    print(f"\n📊 آمار:")
    print(f"  • تعداد جملات اصلی: {result['num_original_sentences']}")
    print(f"  • تعداد جملات خلاصه: {result['num_summary_sentences']}")
    print(f"  • نسبت خلاصه: {result['summary_ratio']:.1%}")
    print(f"  • شماره جملات انتخاب‌شده: {result['selected_indices']}")
    
    print(f"\n✨ خلاصه نهایی:")
    print(result['summary'])
    
    print(f"\n🎯 امتیاز جملات (5 جمله برتر):")
    sorted_scores = sorted(result['scores'].items(), key=lambda x: x[1], reverse=True)
    for i, (idx, score) in enumerate(sorted_scores[:5], 1):
        print(f"  {i}. جمله {idx+1}: {score:.4f}")


def run_all_tests():
    """اجرای تمام تست‌ها"""
    test_with_sample_text()
    test_with_file()
    test_different_ratios()
    
    print("\n" + "=" * 80)
    print("✅ تمام تست‌ها با موفقیت اجرا شدند!")
    print("=" * 80)


if __name__ == "__main__":
    run_all_tests()
